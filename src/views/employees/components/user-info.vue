...
export default {
  ...
  methods: {
    async getUserDetailById() {
      ...
      // need to check if staffPhoto is empty string
      // !"" is true, !" " is false
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // user already has a photo, so upload will be true
    ...
    async saveUser() {
      // need to get staff photo from image-upload instance
      const fileList = this.$refs.staffPhoto.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('Image is loading')
        return
      }
      // must give empty string if no photo
    ...
    async savePersonal() {
      ...
      this.$message.success('Personal detail updated!')
    }
  }
}
</script>
